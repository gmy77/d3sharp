...
namespace Extractor.Packet
{
    public class MooNetHeader
    {
        ...
        public MooNetHeader(byte serviceId, uint methodId, int requestId, uint payloadLength, ulong objectID)
        {
            this.SetData(serviceId, methodId, requestId, payloadLength, objectID);
        }

        public MooNetHeader(CodedInputStream stream)
        {
