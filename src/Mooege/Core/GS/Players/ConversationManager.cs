...
namespace Mooege.Core.GS.Players
{
    ...
    class Conversation
    {
        ...
        {
            var actors = (from a in player.RevealedObjects.Values where a is Mooege.Core.GS.Actors.Actor && (a as Mooege.Core.GS.Actors.Actor).SNOName.SNOId == sno select a);
            if (actors.Count() > 1)
        ...
        private void PlayLine(int LineIndex)
        {
            ...
            player.InGameClient.SendMessage(new PlayConvLineMessage()
            {
                ...
                Params = new PlayLineParams()
                {
                    ...
                    SNOSpeakerActor = GetSpeaker(currentLineNode.Speaker1).SNOName.SNOId,
